...
class ViewController: UIViewController {
	...
	@objc private func letterTapped(_ sender: UIButton) {
		...
		UIView.animate(withDuration: 2, delay: 1, usingSpringWithDamping: 0.5, initialSpringVelocity: 5, options: [], animations:  {
			sender.alpha = 1.0
		}) { finished in
			sender.alpha = 0.0
		}
...
//			button.isHidden = false
	...
	@objc private func loadLevel() {
		...
		DispatchQueue.main.async { [weak self] in
			...
			let newLetterButtons = self?.letterButtons
			if newLetterButtons != nil {
				for btn in newLetterButtons ?? [] {
					btn.isHidden = false
				}
			}
		}
